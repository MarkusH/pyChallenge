...
class PyChallengeGUI(QtGui.QWidget):
    def __init__(self, parent=None):
        ...
        QtCore.QObject.connect(self.qpb1, QtCore.SIGNAL('clicked()'), self.rating)
        QtCore.QObject.connect(self.qpb2, QtCore.SIGNAL('clicked()'), self.compare)
        QtCore.QObject.connect(self.qpb3, QtCore.SIGNAL('clicked()'), self.worst)
        QtCore.QObject.connect(self.qpb4, QtCore.SIGNAL('clicked()'), self.best)

        self.setGeometry(300, 300, 700, 450)
        self.setWindowTitle('pyChallenge - Graphical User Interface')

    def compare(self):
        self.qtext.setText(Popen(["pychallenge.py", "-g", self.qcb.currentText(), "-a", self.qcb2.currentText(), "compare", self.qedit1.text(), self.qedit2.text()], stdout=PIPE).communicate()[0])

    def rating(self):
        self.qtext.setText(Popen(["pychallenge.py", "-g", self.qcb.currentText(), "-a", self.qcb2.currentText(),  "rating", self.qedit1.text()], stdout=PIPE).communicate()[0])

    def worst(self):
        self.qtext.setText(Popen(["pychallenge.py", "-g", self.qcb.currentText(), "-a", self.qcb2.currentText(),  "worst", self.qedit3.text()], stdout=PIPE).communicate()[0])

    def best(self):
        self.qtext.setText(Popen(["pychallenge.py", "-g", self.qcb.currentText(), "-a", self.qcb2.currentText(),  "best", self.qedit3.text()], stdout=PIPE).communicate()[0])
